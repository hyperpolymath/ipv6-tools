...
// Replace ipv6-tools, ipv6-tools, {{DEPS}} with actuals
= ipv6-tools — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining ipv6-tools for
...
| `$PREFIX/share/ipv6-tools/`
| Data files, assets

| `$PREFIX/share/doc/ipv6-tools/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/ipv6-tools/config.toml`

Fallback: `$HOME/.config/ipv6-tools/config.toml`
...
Or via OPSM: `opsm update ipv6-tools`
...
[source,bash]
----
just install --prefix=/opt/ipv6-tools-instance1 --config=/etc/ipv6-tools/instance1.toml
just install --prefix=/opt/ipv6-tools-instance2 --config=/etc/ipv6-tools/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/ipv6-tools/issues
